...
phi = 0.5 * (adag + a) # Flux operator (analogous to position operator)
...
H = 0.5 * ((phi * phi)/l + (Q * Q)/C + hbar*mu*(cos_phi * cos_phi)) # Hamiltonian
L = np.sqrt((C*wo*gamma)/(hbar)) * phi + np.sqrt((gamma)/(C*wo*hbar)) * (1j - (wo/2*cutoff)) * Q # Lindblad operator
...
def time_dep_solver():
    ...
    p_vec = np.ones(n*n, dtype=np.complex_) # Initial density matrix vectorised
    p_arr = [] # Array of density matrix vectors
    superOperatorVal = first_order_equation()
    #print(superOperatorVal)

    for i in range(50):
        p_vec += np.dot(superOperatorVal, p_vec)*p_vec
        p_arr.append(p_vec.reshape(n, n))
    
    return p_arr

if __name__ == "__main__":
    p_arr = time_dep_solver()
    elem = [i[1, 0].real for i in p_arr]
    print(elem)
