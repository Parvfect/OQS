...
% Don't know what these are specifically
A1=0.9348;
x1=0.4464;
p1=-0.07082-0.8887i;
j1=0.07002-0.03285i;
...
for k=1:length(qxs)
    ...
    %e1=e*exp(1i*2*pi*qx);CosQ=real(e1); SinQ=imag(e1); Don't know what e
    %is
    
    HXP=goverw*(3*g^2/2-g+1/2)*(X*P+P*X);
    HXJ=goverw*w/(2*W)*sqrt(beta*v/w)*X*SinQ;
    HPJ= goverw*g^2/2*(sqrt(beta*v/w)*(P*SinQ+SinQ*P)+beta*w/(2*W)*CosQ);
    %HPJ=0;
    ...
    G=kron(ID,AM)+kron(transpose(BM),ID) +kron(transpose(DM),CM)+kron(transpose(FM),EM);
    Solve G*vec[rho]=0
    rhonull=null(G);
    
    rankG=rank(G) ;%should be n^2-1
    
    M=G(1:n^2-1,1:n^2-1);
    rankM=rank(M);%should still be n^2-1

    v1=zeros(1,n^2-1);
    v1(1:n+1:n^2-1)=1;
    v2(1:n^2-1)=G(1:n^2-1,n^2);
    ...
    rho=-M2\transpose(v2);
    rho=[rho;0];%extend rho to n^2 terms
...
save('purityg055W25wn40', 'purityg055W25wn40')
figure
plot(qxs,purity,'kd−');
axis([0 1 0 1])
